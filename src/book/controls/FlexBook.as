...
package book.controls
{
	import book.controls.flexBookClasses.FlexBookEvent;
	import book.controls.flexBookClasses.FlexBookPage;
	import book.utils.UVector;
	...
	public class FlexBook extends DataDrivenControl// implements INavigatorContent
	{
...
//			if(_animatePagesOnTurn)
		...
		private function fillPage(page:book.controls.flexBookClasses.FlexBookPage,pageIndex:Number):void
		{
...
//				trace("flexbook _currentPage :");
				_frontTurningPage.clearContent();
//				trace("flexbook _frontTurningPage :");
				_frontTurningBitmap = null;
				
				_backTurningPage.clearContent();
//				trace("flexbook _backTurningPage :");
...
//-------------------------------------------------------------------------------------------------------		
		...
		private function setupForFlip(x:Number,y:Number,targetPageIndex:Number = NaN):void
		{
			...
			if(_pointOfOriginalGrab.y > (_pageTop + _pageBottom)/2)
			{
				if (_pointOfOriginalGrab.x > _hCenter)
				{
				     var p:Point=new Point(x,_pointOfOriginalGrab.y);
					 var p1:Point=new Point(x+10,_pointOfOriginalGrab.y+1);
					delta =new UVector(p,p1);
				}
				else
				{
					delta = new UVector(new Point(x,_pointOfOriginalGrab.y),new Point(x-10,_pointOfOriginalGrab.y+1));
				}
		...
		private function turnFoldablePage(dragPt:Point, grabPt:Point):void
		{
			...
			*/
			...
			var anchorToDragPt:UVector;
			var dragDistanceFromAnchor:Number;

			if(dragPt.y <= grabPt.y)
			{

				maxDistanceFromAnchor = new UVector(bottomAnchor,grabPt).length;
				...
				anchorToDragPt = new UVector(bottomAnchor,dragPt);
			...
			else 
			{
				...
				maxDistanceFromAnchor = new UVector(topAnchor,grabPt).length;
	
				// make sure we can't pull so far we'd tear the page.  If that happens, just adjust our drag pt and
				// behave as though we weren't pulling father.
				anchorToDragPt = new UVector(topAnchor,dragPt);
				dragDistanceFromAnchor = anchorToDragPt.length;
			...
			var dragToStart:UVector = new UVector(dragPt,grabPt);
			
			
			//determine the normalize vector for the fold.
			var fold:UVector = dragToStart.clone();
			fold.length /= 2;
			...
			{
				var topEdge:UVector = new UVector(new Point(foldIntersectionWithTop,_pageTop), topCorner);
			...
			else
			{
				...
				var foldExtension:UVector = new UVector(foldTopLeft, topCorner);
			...
			{
				var bottomEdge:UVector = new UVector(new Point(foldIntersectionWithBottom,_pageHeight),bottomCorner);
			...
			var leadingEdge:UVector;
			var shortPageEdge:UVector;


			if(_turnDirection == TURN_DIRECTION_FORWARD)
			{
				leadingEdge = new UVector(foldBottomLeft,foldTopLeft);
			}
			else if(_turnDirection == TURN_DIRECTION_BACKWARDS)
			{
				...
				leadingEdge = new UVector(foldBottomRight,foldTopRight);
			...
			if(dragToStart.length2 > .1)
			{
				...
				centerToDrag = new UVector(dragToStartCenter,dragPt);
				m.identity();
...
//					else
				...
				var centerToGrab:UVector = new UVector(dragToStartCenter,grabPt);
				m.identity();
