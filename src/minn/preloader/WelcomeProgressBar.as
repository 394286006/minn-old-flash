package minn.preloader
{
	...
	public class WelcomeProgressBar extends Loader
	{
		//辅助属性，帮助进行进度条的定位
		...
		//进度条的设定，比如显色边框等
		private static var _BarWidth:int = 350;
		private static var _BarHeight:int = 18;
		private static var _BarBackground:uint  = 0xcccccc;
		private static var _BarOuterBorder:uint = 0x737373;
		private static var _BarColor:uint = 0x990000;
...
//			this.loadBytes(new WelcomeScreenGraphic() as ByteArray);
			data = new BitmapData(this.width, this.height, true, 0);
			...
			g.lineStyle(2, _BarOuterBorder, 1, true);
			 px = (this.width - _BarWidth) / 2;
			 py = _TopMargin + _LogoHeight + _Padding;
			g.drawRoundRect(px, py, _BarWidth, _BarHeight, 25,15);
			g.endFill();
		}
...
//			var py : int = _TopMargin + _LogoHeight + _Padding;
        	...
        	g.drawRoundRect(px+1 , py+1 , progWidth, _BarHeight - 3,15,15);
			g.endFill();
        	data.draw(s);
        	return data;
		}
